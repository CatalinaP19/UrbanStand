...
const { 
  ...
  enviarEmailRecuperacion,  // ✅ Agregar esta importación
...
const ValidateAddressEnti = (value) => {
  ...
  if (!direccionRegex.test(direccion)) {
    throw new Error(
      "La dirección debe tener un formato válido para Colombia (ej: Calle 100 #15-55)"
    );
  }
...
router.post(
  ...
  [
    ...
    body("tipoE")
      ...
      .withMessage(
        "Tipo de entidad debe ser: publica, privada, ong, universidad, u otro"
      ),

    body("sector")
      .isIn(["comercio", "investigacion", "gubernamental", "otro"])
      .withMessage(
        "Sector debe ser: comercio, investigacion, gubernamental, u otro"
      ),
    ...
    body("NumTelE")
      .matches(/^3\d{9}$/)
      .withMessage(
        "El número telefónico debe ser válido para Colombia (formato: 3XXXXXXXXX)"
      ),
    ...
    body("direccionE")
      ...
      .withMessage(
        "La dirección debe ser válida para Colombia (ej: Calle 100 #15-55)"
      ),
    ...
    try {
      ...
      if (entidadExistente) {
        return res.status(409).json({
          ...
          message:
            entidadExistente.emailE === emailE
      ...
      res.status(201).json({
        message:
          "Entidad registrada exitosamente. Por favor verifica tu email para confirmar tu cuenta.",
        token,
      ...
      if (error.code === 11000) {
        ...
        return res.status(409).json({
          ...
          message: `Ya existe una entidad con este ${
            field === "emailE" ? "correo institucional" : "NIT"
          }`,
        });
      }
...
router.get("/validate", authenticateEntidadToken, async (req, res) => {
  try {
    const entidad = await EntidadGubernamental.findById(
      req.entidad.entidadId
...
router.get("/estadisticas", authenticateEntidadToken, async (req, res) => {
  try {
    ...
    const estadisticas =
      await EntidadGubernamental.obtenerEstadisticasVendedores(filtros);

    // Estadísticas adicionales
    const totalVendedores = await Vendedor.countDocuments({
      estado_vendedor: { $ne: "eliminado" },
    });
    const vendedoresActivos = await Vendedor.countDocuments({
      estado_vendedor: "activo",
    });
    const vendedoresInactivos = await Vendedor.countDocuments({
      estado_vendedor: "inactivo",
    });
    ...
    res.json({
      ...
      data: {
        resumen: {
          ...
          tasa_actividad:
            totalVendedores > 0
              ? Math.round((vendedoresActivos / totalVendedores) * 100)
              : 0,
        },
    ...
    try {
      ...
      switch (tipo_reporte) {
        case "estadisticas_generales":
          datosReporte =
            await EntidadGubernamental.obtenerEstadisticasVendedores(
              parametros
            );
          break;
...
/// ========================================
// RECUPERACIÓN DE CONTRASEÑA - ENTIDAD
// ========================================
...
router.post('/solicitar-recuperacion', [
  ...
  try {
    ...
    const entidad = await EntidadGubernamental.findOne({ emailE: email });
    ...
    // Guardar token en la entidad
    entidad.tokenRecuperacion = resetToken;
    entidad.tokenRecuperacionExpira = resetTokenExpires;
    await entidad.save();

    // Enviar email usando la función correcta
    try {
      await enviarEmailRecuperacion(  // ✅ Usar enviarEmailRecuperacion
        entidad.emailE,
        entidad.nomEnti,
        resetToken
      );
      
      console.log('✅ Email de recuperación enviado a:', entidad.emailE);
    } catch (emailError) {
      console.error('⚠️ Error al enviar email:', emailError);
      // Revertir cambios
      entidad.tokenRecuperacion = null;
      entidad.tokenRecuperacionExpira = null;
      await entidad.save();
...
router.post('/restablecer-password', [
  ...
  try {
    ...
    // Buscar entidad con token válido y no expirado
    const entidad = await EntidadGubernamental.findOne({
    ...
    console.log('✅ Contraseña actualizada para entidad:', entidad.emailE);

    // Opcional: Enviar email de confirmación de cambio
    try {
      await enviarEmailCambioPassword(entidad.emailE, entidad.nomEnti);
    } catch (emailError) {
      console.error('⚠️ Error al enviar email de confirmación:', emailError);
